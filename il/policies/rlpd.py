...
"""Policy adapter for RLPD agents in this repo's checkpoint layout."""
...
from il.algo.rl.rlpd import ACRLPDAgent
...
@dataclass
class RLPDPolicy:
    """Wrap an `ACRLPDAgent` with the shared policy interface."""
    ...
    def from_checkpoint(
        ...
        *,
        ...
        """Create this repo's RLPD agent and restore a matching checkpoint."""
        ...
        agent = ACRLPDAgent.create(seed, ex_observations, ex_actions, config)
    ...
    def sample_action(self, observation: np.ndarray, *, rng) -> PolicyOutput:
        """Sample one action proposal and actor log-probability."""
        if rng is None:
