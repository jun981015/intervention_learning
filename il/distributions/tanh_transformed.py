...
class TanhTransformedDistribution(tfd.TransformedDistribution):
    """Tanh-transformed distribution with a usable `mode()` method."""

    def __init__(self, distribution: tfd.Distribution, validate_args: bool = False):
        """Wrap a base distribution with a tanh bijector."""
        super().__init__(
            distribution=distribution, bijector=tfb.Tanh(), validate_args=validate_args
        )

    def mode(self) -> jnp.ndarray:
        """Return the tanh of the base distribution mode."""
        return self.bijector.forward(self.distribution.mode())

    @classmethod
    def _parameter_properties(cls, dtype: Optional[Any], num_classes=None):
        """Hide the bijector from TensorFlow Probability parameter introspection."""
