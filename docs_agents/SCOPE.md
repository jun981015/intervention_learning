...
- Learner: RLPD/SAC-style online learner, BC learners, and residual RLPD/TD3 variants
- Expert: restored RLPD/SAC or BC checkpoint policy
...
- Pixel-input policy learning before adding explicit image encoders
...
- `il/algo/rl/residual_rlpd.py`: SAC/RLPD-style residual actor-critic
- `il/algo/rl/residual_td3.py`: deterministic TD3-style residual actor-critic
...
- `il/evaluation/`: generic `evaluate_policy(policy, env, ...)` helper plus TrainContext adapter and optional video rendering
