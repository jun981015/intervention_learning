...
[STATUS_2026-05-21.md](STATUS_2026-05-21.md)를 본다.

## 협업 방식

큰 변경은 Codex가 혼자 길게 진행하지 않는다. 앞으로는 다음 단위로 나눠서 진행한다.

1. 먼저 목표와 변경 범위를 짧게 plan으로 제시한다.
2. 사용자가 동의한 작은 단위만 구현한다.
3. 각 단위가 끝나면 변경 파일, 검증 결과, 남은 리스크를 보고한다.
4. 다음 단위로 넘어가기 전에 다시 확인한다.
5. 사용자가 명시하지 않은 대규모 refactor, config 구조 변경, 학습 run 실행은 하지 않는다.

예외는 typo 수정, 명백한 import/compile error 수정, 문서 링크 보정처럼 작은 정리 작업이다. 그래도 파일을
수정하기 전에는 무엇을 고칠지 먼저 알린다.
